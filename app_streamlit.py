...
if menu_atual not in ["home", "login", "cadastro"]:
    ...
    with st.sidebar:
        ...
        # Botões de navegação protegidos
...
# ==============================================================================
# 3. ROTEADOR DE INTERFACE DO MIOLO (CENTRALIZADO COM st.empty() E st.stop())
# ==============================================================================
...
with miolo_pagina.container():
    ...
    if menu_atual == "home":
        ...
        # Código definitivo de Login com chaves dinâmicas baseadas no UUID/Time
        # 1. CURTO-CIRCUITO DE SEGURANÇA (Se já logou, aborta)
        if "usuario_id" in st.session_state and st.session_state.usuario_id:
            st.session_state.opcao_menu = "💬 Conversar com Lucy"
            st.rerun()

        # 2. INICIALIZA UM IDENTIFICADOR ÚNICO PARA O CONTEÚDO DO LOGIN
        # Isso impede o navegador de reaproveitar o cache visual das outras telas
        if "login_view_uuid" not in st.session_state:
            st.session_state.login_view_uuid = str(time.time()).replace('.', '')

        # Aloca um slot limpo na tela
        ...
        with conteudo_login.container():
            ...
            # ⚡ O SEGREDO: Usamos o UUID no KEY do Form e dos Inputs. 
            # Isso força o navegador a recriar a caixa de login do zero, sumindo com o fantasma.
                    ...
                    try:
                        ...
                        if res:
                            ...
                            if not check_password_hash(banco_password_hash, str(pass_in)):
                                ...
                                # Atualiza status online em transação limpa
                                with conn.cursor() as cursor_update:
                                    cursor_update.execute("UPDATE usuarios SET status = '🟢 Online' WHERE id = %s", (id_numerico,))
                                    conn.commit()
                                
                                # ⚡ LIMPEZA RADICAL DE MEMÓRIA ANTES DE MUDAR A PÁGINA
                                conteudo_login.empty()
                                del st.session_state["login_view_uuid"] # Deleta o UUID para o próximo logout vir limpo
                                
                                # Altera rota e força re-execução imediata
                                st.session_state.opcao_menu = "💬 Conversar com Lucy"
                                st.rerun()
                        else:
            ...
            # Botões inferiores de navegação totalmente desacoplados
